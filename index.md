...
```
分析：RAII 在 c++ 项目中应用广泛，一般用于获取锁或是竞争资源，退出作用域时自动释放。GO 常用 defer 来实现此功能，在函数返回前，按 FILO 的顺序执行注册的 defer 函数。由于 defer 的存在，省去了很多不必要的逻辑，不必每个异常分支都写冗余代码。这也是 GO 实用为主的工程哲学。
