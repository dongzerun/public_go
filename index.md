...
```
* 大学时代，老师教我们 int 是四字节，有符号最大 21 亿，无符号 42 亿。但，现在都 9012 年了，64 位系统己成主流，不必在按字节去节约内存，所以 go 语言默认 64 位的 int 就是 8 字节大小。

* 如果 c 语言中一个变量未初始化，那么值可能是随机的， 因为创建局部变量只是 sp 指针向下移动而己，堆上的变量也不会主动调 memset 清空。但 GO 会直接初始化成零值，从实践中来到实践中去。

* 编绎的可执行文件 GO 会大很多，原因在于 GO 是静态链接，一次编绎，执行时不依赖本机 .so 动态库。写过 python 的肯定遇到过线上机器 .py 库不一致的情况，c/c++ 同样会遇到 .so 库版本问题。
...
```
分析：`genSlice` 分配了一块内存，初始化后返回给调用者，使用后无需主动释放，由 GC 垃圾回收。严格来讲，使用者无法决定某变量在堆还是栈上分配，也可以说 GO 淡化了堆栈的概念与界线。关于什么是栈桢推荐我之前的[分享](https://www.jianshu.com/p/383b15db1f8a)
...
```
分析：RAII 在 c++ 项目中应用广泛，一般用于获取锁或是竞争资源，退出作用域时自动释放。GO 常用 defer 来实现此功能，在函数返回前，按 FILO 的顺序执行注册的 defer 函数。由于 defer 的存在，省去了很多不必要的逻辑，不必每个异常分支都写冗余代码。这也是 GO 实用为主的工程哲学。但是不建义滥用 defer, 尽可能少做业务逻辑。
...
刚工作时，别人封装个 c++ asyncio, 就觉得好高大上，GO 的出现，让这些纷纷跌落神坛，从此**高并发**飞入寻常百姓家。
...
1. 强烈推荐多接触其它语言，尤其是 c/c++, 多做对比，理论上来说 c++, go 脱胎于 c, 都能对内存进行精确控制
